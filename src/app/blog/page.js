        ...
        <ul className="w-full">
          {posts.map((post) => (
            <li key={post.sys.id} className="mb-8 w-full">
              <Link href={`/blog/${post.fields.slug}`} className="flex justify-between items-center w-full p-2 rounded">
                <h2 className="text-lg transition-colors duration-200 hover:text-yellow-400">{post.fields.title}</h2>
                <div className="flex items-center">
                  <span className="mr-2 text-gray-400">{post.fields.tags[0]}</span>
